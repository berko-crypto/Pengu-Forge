...
const admin = new SlashCommandBuilder()
  ...
  .addSubcommandGroup(g => g.setName('set').setDescription('Adjust settings')
    ...
    .addSubcommand(s => s.setName('quality').setDescription('Image quality for casual /generate (cost lever)')
    ...
    .addSubcommand(s => s.setName('campaign-overlay').setDescription('Paste original product pixels over campaign outputs')
      .addStringOption(o => o.setName('value').setDescription('on = pixel-perfect products but can misalign if composition drifts').setRequired(true)
        .addChoices({ name: 'on', value: '1' }, { name: 'off', value: '0' })))
    .addSubcommand(s => s.setName('campaign-quality').setDescription('Image quality for campaigns / drops (default: high)')
      .addStringOption(o => o.setName('value').setDescription('Product/text fidelity needs medium or high').setRequired(true)
        .addChoices({ name: 'low', value: 'low' }, { name: 'medium', value: 'medium' }, { name: 'high', value: 'high' })))
